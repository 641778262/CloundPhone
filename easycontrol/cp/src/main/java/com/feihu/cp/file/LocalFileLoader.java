...
public class LocalFileLoader extends CursorLoader {
    ...
    public LocalFileLoader(Context context, String selectionArgs) {
        super(context);
    ...
    public void parseData(Cursor cursor, final MediaStoreHelper.MediaResultCallback resultCallback) {
        ...
        while (cursor.moveToNext()) {
            ...
            if (!TextUtils.isEmpty(path)) {
                ...
                if (!file.isDirectory() && file.exists() /*&& file.length() > 1024 && file.length() < 20*1000*1000*/) {
                    files.add(file);
                }
            }
        }
     ...
     */
    protected Object getValueFromCursor(Cursor cursor, String columnName, Object defaultValue) {
        try {
